  ...
  <v-card rounded="xl" :color="COLOR.ADMIN.BACKGROUND" variant="outlined" class="mt-1">
    <template v-slot:title>
      <v-chip :color="COLOR.ADMIN.MAIN"
        ><strong>게시글 통계</strong>
        <v-divider vertical class="ml-3 mr-3"></v-divider>
        {{ dashboard.post.labels.at(0) || "" }} ~
        {{ dashboard.post.labels.at(-1) || "" }}
      </v-chip></template
    >
    <template v-slot:append>
      <v-chip :color="COLOR.ADMIN.MAIN" prepend-icon="mdi-note-plus-outline">
        <strong>오늘 {{ dashboard.post.values.at(-1) || 0 }}개 작성됨</strong>
        <v-divider vertical class="ml-3 mr-3"></v-divider>
        최고 {{ Math.max(...dashboard.visit.values) }}개 작성됨
      </v-chip>
    </template>
    ...
    <v-sheet class="mx-auto">
      <v-sparkline
        v-if="dashboard.post.values.length < 20"
    ...
    <v-card-actions class="pa-0 ml-3 mr-3">
      <v-chip :color="COLOR.ADMIN.MAIN" prepend-icon="mdi-note-plus-outline">
        어제 {{ dashboard.post.values.at(-2) || 0 }} 개 작성됨
      </v-chip>
      <v-spacer></v-spacer>
      <v-chip :color="COLOR.ADMIN.MAIN" prepend-icon="mdi-note-plus">
        총 {{ util.num(dashboard.post.total) }} 개 작성됨
      </v-chip>
    </v-card-actions>
  </v-card>
</template>
