    ...
    <v-card class="mx-auto" width="500" color="blue-grey">
      ...
      <v-list>
        ...
        <v-list-item class="pa-0 pl-3">
          <v-checkbox
            v-model="user.permission.writePost"
            density="compact"
            hide-details
            :label="user.permission.writePost ? '글 작성을 할 수 있습니다' : '글을 쓸 수 없습니다'"
          ></v-checkbox>
        </v-list-item>

        <v-list-item class="pa-0 pl-3">
          <v-checkbox
            v-model="user.permission.writeComment"
            density="compact"
            hide-details
            :label="
              user.permission.writeComment ? '댓글 작성을 할 수 있습니다' : '댓글을 쓸 수 없습니다'
            "
          ></v-checkbox>
        </v-list-item>
        ...
        <v-list-item class="pa-0 pl-3">
          <v-checkbox
            v-model="user.permission.sendNote"
            density="compact"
            hide-details
            :label="
              user.permission.sendNote
        ...
        <v-list-item class="pa-0 pl-3">
          <v-checkbox
            v-model="user.permission.sendReport"
            density="compact"
            hide-details
            :label="
              user.permission.sendReport
                ? '다른 회원을 신고할 수 있습니다'
                : '신고 기능을 쓸 수 없습니다'
            "
          ></v-checkbox>
        ...
        <v-list-item>
          <v-textarea
            v-model="user.permission.reason"
            :rules="rules"
            variant="outlined"
            counter
            auto-grow
      ...
      <v-card-actions class="bg-white action">
        <v-btn prepend-icon="mdi-close" @click="user.closeManageUser"
          >아무것도 하지 않고 닫기</v-btn
        >
        <v-spacer></v-spacer>
        <v-btn
